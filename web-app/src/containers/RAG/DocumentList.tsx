...
import { listRAGSources, removeRAGSource } from '../../services/rag'
import { transformMCPSources } from './utils'
import DocumentItem from './DocumentItem'

type SortOption = 'name' | 'date' | 'size' | 'chunks'
type SortDirection = 'asc' | 'desc'

const DocumentList = () => {
  const { documents: ragDocuments, documentsLoading: loading, setDocuments: setRagDocuments, setDocumentsLoading: setLoading } = useRAGDocuments()
  ...
  const handleDeleteDocument = async (sourceId: string) => {
    try {
      ...
      setRagDocuments(ragDocuments.filter(doc => doc.source_id !== sourceId))
    } catch (error) {
      console.error('Failed to delete document:', error)
    }
  }

  const sortDocuments = (docs: typeof ragDocuments) => {
  ...
  const getStatusCounts = () => {
    const counts = ragDocuments.reduce((acc, doc) => {
      acc[doc.status] = (acc[doc.status] || 0) + 1
      return acc
    }, {} as Record<string, number>)
    
    return {
      all: ragDocuments.length,
      ...counts
    }
  }
