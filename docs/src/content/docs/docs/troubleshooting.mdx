...
1. **Verify Hardware and System Requirements**

   **Check GPU Detection:** First, verify that your system recognizes the NVIDIA GPU:
   - **Windows:** Right-click desktop → NVIDIA Control Panel or check Device Manager → Display Adapters
   - **Linux:** Use `lspci | grep -i nvidia`

   **Install Required components:**
   - **NVIDIA Driver:** Install [NVIDIA Driver](https://www.nvidia.com/en-us/drivers/) for your GPU (NVIDIA driver **470.63.01 or higher**). Verify with `nvidia-smi`
   - **CUDA Toolkit:** Download and install [CUDA toolkit](https://developer.nvidia.com/cuda-downloads) (**CUDA 11.7 or higher**). Verify with `nvcc --version`
   - **Linux Additional Requirements:** Install required packages (`sudo apt install gcc-11 g++-11 cpp-11`) and set up CUDA environment
...
2. **Turn on GPU acceleration**
   ...
   Jan manages GPU usage automatically - switches to GPU mode when supported and automatically selects GPU with highest VRAM.
