...
1.
1. **Download Application** - Choose from Stable, Beta, or Nightly releases for Ubuntu (.deb) or other distributions (.AppImage)
2. **Install Application** - For Ubuntu use dpkg or apt-get, for others make the AppImage executable and run it
...
1.
1. **Verify Hardware** - Check GPU detection with `lspci | grep -i nvidia`
2. **Install NVIDIA Driver** - Install from your package manager and verify with `nvidia-smi`
3. **Install CUDA Toolkit** - Install CUDA 11.7+ from package manager and verify with `nvcc --version`
4. **Install Dependencies** - Run `sudo apt install gcc-11 g++-11 cpp-11` and set LD_LIBRARY_PATH
5. **Enable GPU Acceleration** - Go to Settings > Local Engine > Llama.cpp and select appropriate backend
...
See [CUDA Installation Documentation](https://docs.nvidia.com/cuda/cuda-installation-guide-linux/index.html#post-installation-actions) for more details.
