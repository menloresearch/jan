...
1.
1. **Navigate to the Local API Server** - Add an API Key (it can be anything) or fully configure the server at [Server Settings](/docs/api-server#server-settings)
2. **Click Start Server button** - Wait for the confirmation message in the logs panel, your server is ready when you see: `JAN API listening at: http://127.0.0.1:1337`
3. **Test the server** - Click the API Playground button to open its testing interface, select a model from the dropdown menu, and try a simple request
4. **Use the API** - Your server is now ready to receive requests at the configured address and port
