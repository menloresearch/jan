...
1.
1. **Create Model File Structure** - Navigate to the [Jan Data Folder](./data-folder), open `models` folder, and create a new folder for your model
2. **Add Model Files** - Add your `model.gguf` file and create a `model.json` configuration file with proper settings
3. **Configure Settings** - Set the path/location of your model, context length, and chat template in the settings array
4. **Set Parameters** - Configure adjustable settings like temperature, top_p, max_tokens, and penalties that affect model behavior
5. **Test Model** - Restart Jan and verify your model appears in the model selection dropdown


##### Example model.json Configuration

Here's an example configuration for "TinyLlama Chat 1.1B Q4":
...
1. The **Settings** array is where you can set the path or location of your model in your computer, the context length allowed, and the chat template expected by your model.
2. The [**Parameters**](/docs/model-parameters) are the adjustable settings that affect how your model operates or processes the data. Example:
