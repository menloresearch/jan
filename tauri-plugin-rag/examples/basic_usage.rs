...
//! Basic usage example for the Tauri RAG plugin.

use tauri_plugin_rag::{RAGConfig, EmbeddingConfig, ChunkingConfig};
...
fn main() {
    ...
    println!("- tauri_plugin_rag::init() for default configuration");
    println!("- tauri_plugin_rag::init_with_config(config) for custom configuration");
    println!("- tauri_plugin_rag::init_with_config_and_mcp(config) for MCP integration");
}
