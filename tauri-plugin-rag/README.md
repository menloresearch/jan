...
A powerful Tauri plugin for Retrieval-Augmented Generation (RAG) functionality with vector database support, document processing, and Model Context Protocol (MCP) integration.
...
```toml
[dependencies]
tauri-plugin-rag = "2.0.0"
# alternatively with Git:
tauri-plugin-rag = { git = "https://github.com/menloresearch/jan", branch = "main" }
```

Install the JavaScript guest bindings:

```bash
pnpm add @tauri-apps/plugin-rag
# or
npm add @tauri-apps/plugin-rag
# or
yarn add @tauri-apps/plugin-rag
```
...
```rust
use tauri_plugin_rag::{init_with_config, RAGConfig, EmbeddingConfig, ChunkingConfig};
...
```rust
use tauri_plugin_rag::{init_with_config_and_mcp, RAGConfig, EmbeddingConfig};
...
```rust
use tauri_plugin_rag::{Builder, RAGConfig, EmbeddingConfig};
